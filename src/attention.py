...
class BiasedAttention(nn.Module):
    ...
    def __call__(self, x: mx.array, attn_mask: mx.array | None = None) -> mx.array:
        ...
        # Now here instead this sink token will be concatenated with the other tokens, then we get teh softmax and then this sink token is removed from the result (and we are only left with the real tokens)
        attention = sink_softmax(score, self.sinks) @ V  # (B, num_heads, seq_len, d_k)
